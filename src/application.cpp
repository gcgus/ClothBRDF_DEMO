...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
	...
	LUT = Texture::Get("data/brdfLUT.png");

	{
		skybox = new SkyBox(TRUE);
		lights = TRUE;

		node = new SceneNode("Visible node");
		node->mesh= Mesh::Get("data/models/clooth.obj.mbin");
		node->material = node->mats[1];
		node->material->texture = Texture::Get("data/models/ball/albedo.png");



		//luz en la escena
		scene_lightsource = new light();
		scene_ambient.set(0.1, 0.1, 0.1); //Ambient light for the scene
		scene_lightsource->set(Vector3(20, 0, 0), Vector3(0.5, 0.5, 0.5)); //creating a lightsource for the scene
